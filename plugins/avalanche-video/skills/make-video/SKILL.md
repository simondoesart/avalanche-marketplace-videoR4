...
2. **Sync with the account:** run `show_reference_elements` (and `show_characters` if Soul identities exist). Print a table: @name candidate → existing element name, id, created date. **Dedupe:** on same-name duplicates, prefer the newest and say so, or ask (this account has had duplicate `scanner`/`location2`/`Worker-clerk` entries — always check).
3. **Confirm the working set** with the editor: which existing elements to reuse, which need new imagery.
